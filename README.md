...
| [`erdos361/`](erdos361/) | [Erdős Problem 361](https://www.erdosproblems.com/361) — for `f_c(n) = max{\|A\| : A ⊆ [1,⌊cn⌋], n ∉ Σ(A)}`, the sequence `f_c(n)/n` converges **iff `c ≥ 1`**: the `c ≥ 1` exact formula `⌊cn⌋ − ⌈n/2⌉` and, for every `c ∈ (0,1)`, non-convergence — the Erdős–Graham irregularity question | `c ≥ 1` `sorry`-free & axiom-free; irregularity `sorry`-free, conditional on one cited axiom (Alon 1987, Thm 1.1 — see [`erdos361/VERIFICATION.md`](erdos361/VERIFICATION.md) §4); Comparator on CI |
