...
| [`erdos361/`](erdos361/) | [Erdős Problem 361](https://www.erdosproblems.com/361) — for `f_c(n) = max{\|A\| : A ⊆ [1,⌊cn⌋], n ∉ Σ(A)}`, the sequence `f_c(n)/n` converges **iff `c ≥ 1`**: the `c ≥ 1` exact formula `⌊cn⌋ − ⌈n/2⌉` and, for every `c ∈ (0,1)`, non-convergence — the Erdős–Graham irregularity question | `c ≥ 1` `sorry`-free & axiom-free; irregularity `sorry`-free, conditional on one cited axiom (Alon 1987, Thm 1.1 — see [`erdos361/VERIFICATION.md`](erdos361/VERIFICATION.md) §4); **Comparator-certified on CI** |
| [`erdos883/`](erdos883/) | [Erdős Problem 883](https://www.erdosproblems.com/883) — statement of record + sharpness for the first question (odd cycles in the coprime graph): the threshold `⌊n/2⌋+⌊n/3⌋−⌊n/6⌋` admits an odd-cycle-free example, and the ceiling `2(⌊n/3⌋−⌊n/6⌋)+1` is exact and attained one above it. The forcing direction (the manuscript's claimed sharp `c = 1/6` solve) is **stated, not proved** | **Partial**: sharpness `sorry`-free & axiom-free; forcing direction NOT formalized; manuscript's numerical certificates independently reproduced (see [`erdos883/VERIFICATION.md`](erdos883/VERIFICATION.md) §5) |
