...
| [`sendov9-11/`](sendov9-11/) | **Sendov's conjecture, degrees nine to eleven** — every complex polynomial of degree `n` with all zeros in the closed unit disk has, for each zero `a`, a zero of the derivative within distance `1` of `a`. Degree nine was the open case (Brown–Xiang settled `n ≤ 8`; Tao all sufficiently large `n`) | Degree 9: proved — `sorry`-free, axiom-free; **Comparator-certified on CI**. Degrees 10/11: not proved — the certificates are kernel-checked, the analytic reduction is not (see [`VERIFICATION.md`](sendov9-11/VERIFICATION.md)) |
