...
| [`sendov9-11/`](sendov9-11/) | **Sendov's conjecture, degrees nine to eleven** — every complex polynomial of degree `n` with all zeros in the closed unit disk has, for each zero `a`, a zero of the derivative within distance `1` of `a`. Degrees nine to eleven were open (Brown–Xiang settled `n ≤ 8`; Tao all sufficiently large `n`, ineffectively) | Degrees 9, 10 and 11: **proved** — `sorry`-free, axiom-free, unconditional; the analytic reductions and all 582+19 certificates are machine-checked. Degree 9 is **Comparator-certified on CI**; the degree-10/11 configs are shipped but **not yet run** (see [`VERIFICATION.md`](sendov9-11/VERIFICATION.md) §18, §20) |
