/-
  Comparator bridge — second-moment master.

  Proves each `Challenge.lean` statement by delegating to the fully verified
  theorem in `masters/Erdos1054_2ndMomentProof.lean`.
  ...
  `comparator/config/2ndMoment.json` can compare `Challenge` against `Solution`.
-/
...
theorem erdos1054_main (A : Nat) (hA : 1 ≤ A) :
  ...
  Erdos1054.erdos1054_second_moment_full_proof A hA
