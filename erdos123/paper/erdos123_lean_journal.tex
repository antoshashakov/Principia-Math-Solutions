...
stronger statement that the Bernoulli subset sums of $\Sg\cap[x,\rho x)$ satisfy a
...
\[
 \Sx=\Sg\cap[x,\rho x),\qquad
 W_x=\sum_{s\in\Sx}s,\qquad V_x^2=\sum_{s\in\Sx}s^2,
\]
and
\[
 \mu_x=\frac{W_x}{2},\qquad \sigma_x=\frac{V_x}{2}.
\]
Let $(\xi_s)_{s\in\Sx}$ be independent Bernoulli variables taking the values
$0,1$ with equal probability, and put $Y_x=\sum_{s\in\Sx}s\xi_s$.
...
every integer $n$ with $|n-\mu_x|\le\sigma_x$ is a subset sum of $\Sx$.
\end{theorem}
...
\]
This set parametrizes $\Sx$.  The ratios of any two of
...
\begin{proposition}[Geometry and moments]\label{prop:grid}
...
\end{equation}
Moreover, for some $n\asymp L$ there are distinct elements $s_v\in\Sx$
indexed by
\[
...
Since $|\Delta_n^\circ|\asymp n^2$, this gives $|\Sx|\gg L^2$.  Conversely,
...
Thus $|\Sx|\ll L^2$.  Finally $x\le s<\rho x$ for every $s\in\Sx$, and
...
$\distz t\le\delta_0/x$.  To sharpen the constant, choose a rational $\rho_0$ with
$1<\rho_0\le\min(\rho,3/2)$ and repeat the preceding argument for
$\Sg\cap[x,\rho_0x)$.  Writing $t=r+u$, we then have $|u|\le1/(32x)$;
nearest-integer rounding vanishes on this set, and hence
...
\begin{equation}\label{eq:tail}
 ...
 \left|\prod_{s\in\Sx}\cos(\pi st)\right|dt
...
$T/V_x\le |t|\le2\delta/x$, every $s\in\Sx$ satisfies
$|st|<2\delta d\le1/4$, so \eqref{eq:cos-decay} and
$V_x^2=\sum s^2$ give
\[
 \prod_{s\in\Sx}|\cos(\pi st)|\le \exp(-2V_x^2t^2).
\]
...
\begin{equation}\label{eq:product-gaussian}
 \left|\prod_{s\in\Sx}\cos(\pi st)
 -\exp\!\left(-\frac{\pi^2V_x^2t^2}{2}\right)\right|
 \le \pi^4t^4\sum_{s\in\Sx}s^4.
\end{equation}
...
$n$ is a subset sum of $\Sx$.
\end{proof}

\begin{proof}[Proof of Theorem~\ref{thm:main}]
First suppose $\rho=p/q$ is rational.  By Proposition~\ref{prop:grid},
$|\Sx|\ge4p^4$ for all sufficiently large $x$, and hence
...
$p(x+1)$; elements leaving $\Sx$ only decrease the sum.
...
since $\Sx$ is nonempty for all sufficiently large $x$, and then $W_x\ge x$.  By
